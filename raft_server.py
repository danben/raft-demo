...
class Servicer(raft_pb2_grpc.RaftServicer):
    ...
    append_entries_handler: Callable[[AppendEntriesRequest],
                                     AppendEntriesResponse]

    async def RequestVote(
            self, request: VoteRequest,
            context: grpc.aio.ServicerContext) -> RequestVoteResponse:
        return self.request_vote_handler(request)

    async def AppendEntries(
            self, request: AppendEntriesRequest,
            context: grpc.aio.ServicerContext) -> AppendEntriesResponse:
...
class Server:
    ...
    def __post_init__(self: Self) -> None:
        servicer: Servicer = \
                Servicer(request_vote_handler=self.request_vote_handler,
