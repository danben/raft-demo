...
class Node:
    ...
    def __post_init__(self: Self) -> None:
        self._raft_rpc_server = raft_server.Server(
    ...
    async def _load_persistent_state(self: Self) -> None:
        ...
        if os.path.isfile(self.config.persistent_state_file):
            with open(self.config.persistent_state_file, "r") as f:
                ...
                self._print(
                    f"Initialized persistent state from disk: "
        ...
        self._print(f"Last index and term: {self._log.last_index_and_term()}")
    ...
    @check_term
    def _request_vote_handler(
        self: Self, vote_request: VoteRequest
    ) -> RequestVoteResponse:
        ...
        if vote_request.term >= self._current_term and (
            self._voted_for is None or self._voted_for == requester_id
        ):
    ...
    @check_term
    def _append_entries_handler(
        self: Self, append_entries_request: AppendEntriesRequest
    ) -> AppendEntriesResponse:
        """Iff the requester is a viable leader (based on term) and the term
        for our entry at prevLogIndex matches the term specified in
        prevLogTerm, we can go ahead and update our log with the new entries.
        Additionally, if the leader has sent us a higher commit index than the
        one that we know, we can update our own and apply the relevant entries.
        """
        if append_entries_request.log_entries:
            self._print(
                f"Received {len(append_entries_request.log_entries)} "
                "log entries from "
                f"{append_entries_request.requester_id}, starting with index "
                f"{append_entries_request.log_entries[0].index}"
            )
        if (
            append_entries_request.term >= self._current_term
            ...
            if append_entries_request.leader_commit > self._log.commit_index():
                ...
                self._print(
                    f"Got new commit index from leader: {new_commit_index}"
                )
    ...
    def _get_value_handler(self: Self, key: Key) -> GetValueResponse:
        return GetValueResponse(
            key=key.key, value=self._log.get_value(key.key)
        )
    ...
    async def _append_entries_on_one_server(self: Self, server: int) -> int:
        ...
        while True:
            ...
            entries_to_send: list[LogEntry] = self._log.get_entries_from(
                next_index_for_this_server
            )

            try:
                async with asyncio.timeout(timeout):
                    prev_log_index = next_index_for_this_server - 1
                    resp: AppendEntriesResponse = await self._raft_rpc_clients[
                        server
                    ].append_entries(
                        term=self._current_term,
                        leader_id=self.config.raft_server_port,
                        prev_log_index=prev_log_index,
                        prev_log_term=self._log.term_for_index(prev_log_index),
                    ...
                    if resp.success:
                        ...
                        self._state.match_index[server] = entries_to_send[
                            -1
                        ].index
                        return server
                    else:
                ...
                self._print(
                    f"Append entries request to server {server} failed; "
                    "retrying in 2s"
                )
                await asyncio.sleep(2)
            except TimeoutError:
                self._print(
                    f"Append entries request to server {server} timed "
                    "out; retrying"
                )
            except Exception as e:
                self._print(
                    f"Append entries request to server {server} " f"raised {e}"
                )
    ...
    async def _append_entries(self: Self) -> None:
        ...
        match self._state:
            case FollowerState() | CandidateState():
                self._print(
                    "Bug: attempting to append entries while not in "
                    "Leader state"
                )
                sys.exit(1)
            case LeaderState(append_entries_tasks=append_entries_tasks):
                replications: int = 1
                ...
                for server in self.config.all_raft_server_ports:
                    if server != self.config.raft_server_port:
                        ...
                        append_entries_tasks[server] = server_task
    ...
    def _update_commit_index(self):
        ...
        """
        assert isinstance(self._state, LeaderState)
        match_indices = sorted(self._state.match_index.values())

        # This implicitly counts our own commit index as the highest value
        halfway_point = int(len(self.config.all_raft_server_ports) / 2)

        min_with_majority = match_indices[halfway_point]
        if (
            min_with_majority > self._log.commit_index()
    ...
    async def _propose_mapping_handler(
        ...
        match self._state:
            case FollowerState(leader=leader):
                return ProposeMappingResponse(
                    success=False, current_leader=leader
                )
            case CandidateState():
                return ProposeMappingResponse(
                    success=False, current_leader=None
                )
            case LeaderState():
                self._print(
                    "Received mapping proposal: "
                    f"{mapping.key}={mapping.value}"
                )
                self._log.add_new_entry(
                    term=self._current_term,
                    key=mapping.key,
                    value=mapping.value,
                )
                ...
                self._print(
                    f"Mapping {mapping.key}={mapping.value} "
                    "successfully replicated; updating commit index"
                )
                self._update_commit_index()
                return ProposeMappingResponse(
                    success=True, current_leader=self.config.raft_server_port
                )
            case _:
                assert False
    ...
    async def _wait_for_next_heartbeat(self: Self) -> None:
        ...
        match self._state:
            ...
            case FollowerState(receive_heartbeats_task=recv_heartbeats_task):
                if recv_heartbeats_task is not None:
                    # self._print('Heartbeat received. Cancelling old task')
                    recv_heartbeats_task.cancel()

                wait_for_heartbeat = asyncio.sleep(
                    self.config.heartbeat_timeout_ms / 1000
                )
                self._state.receive_heartbeats_task = loop.create_task(
                    wait_for_heartbeat
                )
    ...
    async def _hold_election(self: Self) -> None:
        ...
        for coro in asyncio.as_completed(vote_requests):
            try:
                ...
                # Can't reach one of the other servers; ignore. Either we'll
                # win the election anyway or try again
                pass
    ...
    async def _election_loop(self: Self) -> None:
        ...
        election_timeout_ms = random.randrange(
    ...
    def _send_heartbeats(self: Self) -> None:
        ...
        for other_id, client in self._raft_rpc_clients.items():
            heartbeat_coro = client.append_entries(
                term=self._current_term,
                leader_id=self.config.raft_server_port,
                prev_log_index=prev_log_index,
