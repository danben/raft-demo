...
from raft_pb2 import GetValueResponse, Key
from raft_pb2 import Mapping, ProposeMappingResponse


@dataclass(kw_only=True)
class Servicer(raft_pb2_grpc.ClusterServicer):
    get_value_handler: Callable[[Key], GetValueResponse]
    propose_mapping_handler: Callable[[Mapping],
                                      Coroutine[Any, Any,
                                                ProposeMappingResponse]]

    async def GetValue(
            self, request: Key,
            context: grpc.aio.ServicerContext) -> GetValueResponse:
        return self.get_value_handler(request)

    async def ProposeMapping(
            self, request: Mapping,
            context: grpc.aio.ServicerContext) -> ProposeMappingResponse:
...
class Server:
    ...
    def __post_init__(self: Self) -> None:
        servicer: Servicer = \
                Servicer(get_value_handler=self.get_value_handler,
